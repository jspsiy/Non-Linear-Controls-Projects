...
clear all
close all
global tau tilde eps a K K2 beta
tau=[];
tilde=[];
eps=[];
...
wyd=[X(40:44)';X(45:49)'];
P=[X(50:54)';X(55:59)';X(60:64)';X(65:69)';X(70:74)'];
% Select gains for controller
global a K  beta   ;%0.15; %Enter a number
...
global tau tilde eps K2
tilda=theta-thetaHat;
tau=[tau;t(length(t)),u'];
tilde=[tilde;t(length(t)),tilda'];
eps=[eps;t(length(t)),Epsilon'];

rDot        = M^-1*(-Vm*qDot-fd*qDot+u)-qdDotDot+a*eDot; %Enter the expression
thetaHatDot =P*transpose(Y)*r+K2*P*transpose(Yf)*Epsilon;% eye(5)*transpose(Yd)*r;%Enter the expression

PDot=-P*(transpose(Yf)*Yf)*P;
PDotMat=[PDot(1,:)';PDot(2,:)';PDot(3,:)';PDot(4,:)';PDot(5,:)'];
% Stacked dynamics vector (XDot is the same size and "form" as X)
XDot        = [eDot;rDot;thetaHatDot;ohmyMatDot;
    wyMatDot;ohmydMatDot;wydMatDot;PDotMat;ufDot];
end