...
rDot        = M\(-Vm*qDot-fd*qDot-fs*sign(qDot)+u)-qdDotDot+a*eDot; %Enter the expression
...
subplot(2,2,3)
plot(timestack,repmat(theta(1:5),1,length(timestack)),'-','LineWidth',2)
hold on
ax = gca;
ax.ColorOrderIndex = 1;
plot(timestack,thetaHat(1,:),':','LineWidth',2)
plot(timestack,thetaHat(2,:),':','LineWidth',2)
plot(timestack,thetaHat(3,:),':','LineWidth',2)
plot(timestack,thetaHat(4,:),':','LineWidth',2)
plot(timestack,thetaHat(5,:),':','LineWidth',2)
legend('hat1','hat2','hat3','hat4','hat5')
title('Thetahats');
hold off
...
plot(tilde(:,1),tilde(:,7))
plot(tilde(:,1),tilde(:,8))
legend('tilde1','tilde2','tilde3','tilde4','tilde5','tilde6','tilde7')
title("thetatilde")

figure(3)
plot(timestack,repmat(theta(6:7),1,length(timestack)),'-','LineWidth',2)
hold on
ax = gca;
ax.ColorOrderIndex = 1;
plot(timestack,thetaHat(6,:),':','LineWidth',2)
plot(timestack,thetaHat(7,:),':','LineWidth',2)
legend('hat6','hat7')
title('Static friction Adaptation');
hold off


% figure(4)
% plot(timestack,repmat(theta,1,length(timestack)),'-','LineWidth',2)
% hold on
% ax = gca;
% ax.ColorOrderIndex = 1;
% plot(timestack,thetaHat(1,:),':','LineWidth',2)
