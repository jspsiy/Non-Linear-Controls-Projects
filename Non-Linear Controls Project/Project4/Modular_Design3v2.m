...
f1       = 5.3;
f2       = 1.1;
global tau tilde eps e20 thetahat0 thetahatplot thetaHatMat;
tau=[];
tilde=[];
eps=[];
thetaHatMat=[];
...
thetaHat=zetaHat;%+ones(2,5)'*e1;%+norm(e1)*ones(5,1)+norm(e2)*ones(5,1);
%thetaHatMat=[thetaHatMat,[t;thetaHat(1);thetaHat(2);thetaHat(3);thetaHat(4);thetaHat(5)]];
u        = Yd*thetaHat+(K+1)*(e2-e20)+mew;
%u        = mew;%((K+1)*e2 - (K+1)*e20 + mew);%+Y*thetaHat;%-K*r; % -e Enter the expr..ession
e2Dot=qdDD-M\(-Vm*qDot-fd*qDot+u)+a1*e1Dot;
%r=e2Dot+a2*e2;
...
zetaHatDot=gamma*Yd'*e2+gamma*Yd'*(e2-e20+(K+1)^-1*mew);%+gamma*Yd'*e1+gamma*Yd'*e2;
